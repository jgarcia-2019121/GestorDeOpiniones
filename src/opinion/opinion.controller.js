...
export const update = async (req, res) => {
    try {
        ...
        let updatedOpinion = await Opinion.findOneAndUpdate(
            { _id: id },
            data,
            { new: true }
        )
...
export const deleteO = async (req, res) => {
    try {
        let { id } = req.params
        let deletedOpinion = await Opinion.findOneAndDelete({
            $and: [
                { user: data.user },
...
export const search = async (req, res) => {
    try {
        let { search } = req.body
        let opinion = await Opinion.find({ titulo: search })
